...
int query(int l,int r)
{
	...
	{
		if(l%2 == 0) res+=tree[l+1];	//jeśli l to prawy syn
		if(r%2 == 1) res+=tree[r-1];	//jeśli r to lewy syn
		l/=2;		//przejście wyżej
		r/=2;		//przejście wyżej 
	}
	return res;		
...
int main()
{
	...
	{
		char znak=A[i];
		int pos=chars[znak].back();		//ostatnia wartość vectora (B) (przed obróceniem była to pierwsza wartość)

		ans+=pos-1-query(1,pos);	//pozycja wystąpienia w B - pozycja wystąpienia w A + drzewo
