...
namespace gameplay{
    ...
    pii choose_best_gift(){
        ...
        for(auto u: argmini){
            for(auto w: argmini){
                if(u == w) continue;
                pii center = {(gift::gift[u].x + gift::gift[w].x) / 2, (gift::gift[u].y + gift::gift[w].y) / 2};
                double d_x = center.fi - gift::gift[u].x;
                double d_y = center.se - gift::gift[u].y;
                int dist = std::ceil(std::sqrt(d_x*d_x + d_y*d_y));
                if(dist > 30) continue;

                d_x = center.fi - gift::gift[w].x;
                d_y = center.se - gift::gift[w].y;
                dist = std::ceil(std::sqrt(d_x*d_x + d_y*d_y));
                if(dist <= 30) return center;
            }
        }

        return std::make_pair(gift::gift[argmini.front()].x, gift::gift[argmini.front()].y);
    }
