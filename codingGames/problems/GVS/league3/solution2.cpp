...
namespace gameplay{
    ...
    int choose_best_gift(){
        int mini = inf, argmini = max_gift_no - 1, val = -1;
        for(int i = 0; i < gift::end; i++){
            if(player::bot[me][0].cooldown <= 2){
                if(player::gift_intercetpion_time[me][0][i] != never && player::gift_intercetpion_time[me][0][i] <= mini && player::gift_intercetpion_time[me][0][i] - 2 <= player::gift_intercetpion_time[foe][0][i] and gift::gift[i].value > val)
                    mini = player::gift_intercetpion_time[me][0][i], argmini = i, val = gift::gift[i].value;
            }
            else{
                if(player::gift_intercetpion_time[me][0][i] != never && player::gift_intercetpion_time[me][0][i] <= mini && player::gift_intercetpion_time[me][0][i] <= player::gift_intercetpion_time[foe][0][i] and gift::gift[i].value > val)
                    mini = player::gift_intercetpion_time[me][0][i], argmini = i, val = gift::gift[i].value;
            }
        }
...
namespace player{
    ...
    bool fireball(){
        ...
        if(player::bot[me][0].cooldown) return 0;

        int best_id = -1, best_val = -1;
        for(int i = 0; i < gift::end; i++){
            double d_x = gift::gift[i].x - bot[me][0].x;
            double d_y = gift::gift[i].y - bot[me][0].y;
            int dist = std::ceil(std::sqrt(d_x*d_x + d_y*d_y)); 

            if(player::gift_intercetpion_time[foe][0][i] == 1 and player::gift_intercetpion_time[me][0][i] and dist <= 300 and gift::gift[i].value > best_val){ // fireball it
