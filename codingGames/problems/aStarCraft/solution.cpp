...
bool empty_tiles_grid[N][M]; // is in this place element of empty_tiles vector
...
bool neighbour_of_empty_tile_element(int x, int y){
    for(auto u: dir){
        u.x += x;
        u.y += y;
        if(u.x < 0 || u.x >= N || u.y < 0 || u.y >= M) continue;
        if(tiles_contain(u.x, u.y)) return 1;
    }
...
struct Solution{
    ...
    void generate_arrangemetn(){
        for(int i = 0; i < empty_tiles.size(); i++)
            arrangement[i] = possible_moves[i][0];
    }
    ...
    void find_empty_tiles(){
        for(int i = 0; i < N; i++){
            for(int j = 0; j < M; j++){
                ...
                empty_tiles_grid[i][j] = true;
            }
        }

        // add starting position of robots
        for(auto [x, y, dir]: robots)
