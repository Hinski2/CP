...
public:
    ...
    void make_tunels(){
        ...
        while(resorces > cost_of_pod){
            int lp = min(3, (int) landing_pads_id.size());
            int lm = min(1, (int) lunar_modules_id.size());

            vector<int> chosen_id;
            for(int i = 0; i < lp; i++)
                chosen_id.push_back(rand() % landing_pads_id.size());
            for(int i = 0; i < lm; i++)
                chosen_id.push_back(rand() % lunar_modules_id.size());

            // make all possible connections with choosen buildings
            vector<connection_pts> poss;
            for(int i = 0; i < 20; i++)
                for(auto u: chosen_id){
                    int x = rand() % num_of_buildings;
                    if(x == u) continue;
                    if(ok_connection(u, x)) poss.push_back({evaluate_connection(x, u), x, u});
                }

            // chose the best 
            sort(poss.begin(), poss.end(), cmp_for_connection_pts);

            for(int i = 0, j = 0; i < poss.size() && j < 3; i++){
                int &a = poss[i].b1;
                int &b = poss[i].b2;
