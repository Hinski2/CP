...
namespace game{
    ...
    inline void print_ans(){
        ...
        if(chosen_body_part.type == HARVESTER) std::cout << string_of_dir[chosen_body_part.dir];
        std::cout << std::endl;
    }
    ...
    inline void choose_body_part(){
        ...
        for(int i = 0; i < me.protein_wells.size; i++){
            ...
            for(auto d: DIR){
                d.fi += protein_well.x, d.se += protein_well.y;
                if(entity_matrix[d.fi][d.se].type != NOTHING and entity_matrix[d.fi][d.se].owner == FOE_ORGAN){
                    // our well in in danger
