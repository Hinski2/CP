...
bool check(int mid) {
    ...
    for(auto city: cities) {
        while(tower_idx < towers.size() and abs(towers[tower_idx] - city) > mid)
            tower_idx++;

        if(tower_idx == towers.size() or abs(towers[tower_idx] - city) > mid)
            return 0;
    }
...
signed main(){
    ...
    int n, m; cin >> n >> m;
    cities.resize(n); cin >> cities;
    towers.resize(m); cin >> towers;
