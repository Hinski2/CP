...
int get_min(string s, char team) {
    ...
    int score[2] = {0}, play[2] = {5, 5};
    for(int i = 0; i < s.size(); i++) {
        score[i % 2] += s[i] == '1';
        play[i % 2]--;

        if(score[i % 2] > score[(i + 1) % 2] + play[(i + 1) % 2]) return i + 1;
        if(score[(i + 1) % 2] > score[i % 2] + play[i % 2]) return i + 1;
    }
...
int solve() {
    string s; cin >> s;
    return min(get_min(s, '1'), get_min(s, '0'));
}
