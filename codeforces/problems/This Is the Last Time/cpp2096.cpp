...
struct Casino {
    ...
    friend istream& operator >> (istream& in, Casino &cs) {
        in >> cs.l >> cs.r >> cs.real;
        return in;
    }
};

int solve() {
    int n, cash; cin >> n >> cash;
    vector<Casino> v(n); cin >> v;
    sort(all(v));

    for(auto cs: v) {
        if(cs.l <= cash and cash <= cs.r and cash < cs.real) cash = cs.real;
    }
