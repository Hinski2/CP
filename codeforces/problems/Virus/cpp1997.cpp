...
int solve() {
    int n, m; cin >> n >> m;
    vector<int> infected(m); cin >> infected;
    sort(all(infected));

    vector<int> segments(m);
    segments[0] = n - infected[m - 1] + infected[0] - 1;

    for(int i = 0; i + 1 < m; i++) {
        segments[i + 1] = infected[i + 1] - infected[i] - 1;
    }

    sort(all(segments), greater<int>());

    int t = 0, ans = 0;
    for(auto segment: segments) {
        int len = segment - 2*t;
        if(len <= 0) return n - ans;
